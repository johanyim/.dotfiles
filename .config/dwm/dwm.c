...
//TODO : understand logic of comboview and pertag view to implement the same here
void
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		for (c = m->clients; c; c = c->next) {
			if (c->tags & (1 << i)) {
				drw_rect(drw, x, 1 + (indn * 4), selmon->sel == c ? 6 : 2, 2, 1, urg & 1 << i);
				indn++;
			}
		}
		x += w;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
    ...
    if (shouldcenteralone(c)) {
        //can take away the 1.0 - x, but this gives more predictable behaviour?
        wc.x = c->x = (c->mon->mw - (c->mon->mw * (1.0 - c->mon->mfact) ) ) / 2 ;  
        wc.y = c->y = (c->mon->wy) + (c->mon->gappov) + (c->bw);
        wc.width  = c->w = (c->mon->mw) * (1.0 - c->mon->mfact) ;
...
view(const Arg *arg)
{
    ...
    // TODO: Do the same thing with comboview and combotag
